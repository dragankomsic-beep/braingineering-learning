...
const ADMIN_PASSWORD = 'braing2026admin';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (password !== ADMIN_PASSWORD) {
      return res.status(200).json({ error: 'Falsches Passwort.' });
    }

    // Get all used codes from blob
    const { blobs } = await list({ prefix: 'used-codes/' });
    const usedMap = {};
    for (const blob of blobs) {
      // Extract code from pathname: "used-codes/XXXX.json"
      const match = blob.pathname.match(/used-codes\/([A-Z0-9]+)\.json/);
      if (match) {
        usedMap[match[1]] = blob.uploadedAt || 'unbekannt';
      }
    }

    const codes = ALL_CODES.map(c => ({
      code: c,
      used: !!usedMap[c],
      usedAt: usedMap[c] || null
    }));

    const usedCount = codes.filter(c => c.used).length;
    const freeCount = codes.length - usedCount;

    return res.status(200).json({ codes, usedCount, freeCount, total: codes.length });
  } catch (err) {
    console.error('admin error:', err);
    return res.status(200).json({ error: 'Fehler: ' + err.message });
  }
}
